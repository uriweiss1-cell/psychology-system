...
router.get('/', (req, res) => {
  res.json(db.get('supervisions').value());
});
...
router.post('/', (req, res) => {
  ...
  db.get('supervisions').push(s).write();
...
router.put('/:id', (req, res) => {
  ...
  const s = db.get('supervisions').find({ id }).value();
  ...
  db.get('supervisions').find({ id }).assign(update).write();
  res.json(db.get('supervisions').find({ id }).value());
});

router.delete('/:id', (req, res) => {
  db.get('supervisions').remove({ id: +req.params.id }).write();
  res.json({ ok: true });
});
